...
import { AddCandidateModal, NewCandidateFormData } from './AddCandidateModal';
...
export const CandidateListPage: React.FC<CandidateListPageProps> = ({
  ...
  const handleCreateSubmit = async (formData: NewCandidateFormData) => {
    if (isSubmittingCandidate) return;
    setIsSubmittingCandidate(true);
    try {
      const docItems = (formData.docText.trim() || formData.docUrl) ? [
        {
          id: `doc-${Date.now()}`,
          title: formData.docTitle.trim() || `${formData.name}_지원서.${formData.docType === 'url' ? 'link' : formData.docType}`,
          type: formData.docType as any,
          contentSnippet: (formData.docText || formData.docUrl || '지원 서류').substring(0, 100),
          rawText: formData.docText || (formData.docUrl ? `외부 링크: ${formData.docUrl}` : ''),
          url: formData.docUrl || undefined,
          fileData: formData.docUrl?.startsWith('data:') ? formData.docUrl : undefined,
          fileSize: '첨부 서류',
      ...
      await onAddCandidate({
        ...
        track: formData.fieldNote.trim() || '일반',
        studentId: formData.studentId || `2026${Math.floor(10000 + Math.random() * 90000)}`,
        phone: formData.phone || '010-0000-0000',
        email: `${formData.name.trim().toLowerCase()}@smartlab.edu`,
        timeslot: {

...
// GET /api/audit-logs & /api/admin/audit-logs
const handleGetAuditLogs = (req: any, res: any) => {
  res.json(db.auditLogs || []);
};
adminRouter.get('/audit-logs', handleGetAuditLogs);
adminRouter.get('/admin/audit-logs', handleGetAuditLogs);
...
adminRouter.post('/verify-password', handleVerifyPassword);
adminRouter.post('/admin/verify-password', handleVerifyPassword);
...
adminRouter.post('/unlock-edit', handleUnlockEdit);
adminRouter.post('/admin/unlock-edit', handleUnlockEdit);

// POST /api/admin/modify-evaluation - Direct score edit by admin
const handleModifyEvaluation = async (req: any, res: any) => {
...
adminRouter.post('/modify-evaluation', handleModifyEvaluation);
adminRouter.post('/admin/modify-evaluation', handleModifyEvaluation);

// POST /api/admin/complete-all-interviews
const handleCompleteAllInterviews = async (req: any, res: any) => {
  try {
    const {
      operatorName,
    ...
    db.notifications.unshift({
      ...
      message: `관리자가 전체 면접 평가를 공식 완료 처리했습니다. (결과 공개: ${
        db.settings.isResultsPublished ? '공개됨' : '비공개'
      }, 합불공개: ${db.settings.showPassFailToCandidates ? 'ON' : 'OFF'}, 면접관표시: ${
        db.settings.interviewerNameDisplayPolicy
      })`,
      timestamp: getKSTTimeStr(),
      createdAt: Date.now()
    });
...
adminRouter.post('/complete-all-interviews', handleCompleteAllInterviews);
adminRouter.post('/admin/complete-all-interviews', handleCompleteAllInterviews);
...
adminRouter.post('/publish-results', handlePublishResults);
adminRouter.post('/admin/publish-results', handlePublishResults);
